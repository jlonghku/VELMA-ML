...
class SurrogateModel(nn.Module):
    """
    简化后的代理模型，用于基于时间序列和VELMA参数预测输出。

    参数：
    - time_series_input_size: 时间序列数据的输入特征数。
    - lstm_hidden_size: 共享LSTM的隐藏单元数。
    - velma_param_size: VELMA参数的数量。
    - shared_lstm_layers: 共享LSTM的层数。
    - fc_hidden_dims: 定义共享全连接层的隐藏层大小列表。
    - output_size: 最终输出的大小。
    - use_dropout: 是否在层中使用dropout。
    - dropout_prob: dropout的概率。
    """
    def __init__(self, time_series_input_size=2, lstm_hidden_size=64, velma_param_size=1, 
                 shared_lstm_layers=2, fc_hidden_dims=[256, 128, 64], output_size=1, 
    ...
    def forward(self, time_series, velma_params):       
        ...
        fc_out = self.fc_layers(combined_input)  # 形状: [batch_size * seq_len, last_fc_dim]
        fc_out = fc_out.view(batch_size, seq_len, -1)  # 恢复时间序列形状: [batch_size, seq_len, last_fc_dim]

        # 生成最终输出
        output = self.output_layer(fc_out)  # 形状: [batch_size, seq_len, output_size]
        return output

class SurrogateModel_LSTM(nn.Module):
    """
    使用多变量 LSTM 的代理模型，直接在模型内部实现多步预测，生成完整的时间序列输出。
    
    参数：
    - time_series_input_size: 时间序列数据的输入特征数（多变量）。
    - lstm_hidden_size: LSTM 的隐藏单元数。
    - velma_param_size: VELMA 参数的数量。
    - num_lstm_layers: LSTM 层数。
    - fc_hidden_dims: 定义共享全连接层的隐藏层大小列表。
    - output_size: 最终输出的大小。
    - use_dropout: 是否在层中使用 dropout。
    - dropout_prob: dropout 的概率。
    """
    def __init__(self, time_series_input_size=2, lstm_hidden_size=64, 
                 velma_param_size=1, num_lstm_layers=2, 
                 fc_hidden_dims=[128, 64], output_size=1, use_dropout=True, dropout_prob=0.2):
        super(SurrogateModel_LSTM, self).__init__()
        
        # 更新输入的维度，包括时间序列特征和前多步的输出
        self.time_series_input_size = time_series_input_size + output_size

        # 创建 LSTM 层
        self.lstm = nn.LSTM(input_size=self.time_series_input_size, hidden_size=lstm_hidden_size, 
                            num_layers=num_lstm_layers, batch_first=True, dropout=dropout_prob if use_dropout else 0)
        
        # LSTM 输出与 VELMA 参数的组合输入大小
        self.combined_input_size = lstm_hidden_size + velma_param_size
    ...
    def forward(self, time_series, output_history, velma_params):
        time_series_len = time_series.size(1)
        output_history_len = output_history.size(1)
        
        # 预分配输出张量 [batch_size, total_len, output_size]
        all_outputs = torch.zeros((output_history.size(0), time_series.size(1), output_history.size(2)),
                                  device=output_history.device)
        all_outputs[:, :output_history_len, :] = output_history  # 填入初始 output_history

        for i in range(time_series_len - output_history_len):
            # 准备当前时间步的输入
            combined_input = torch.cat((time_series[:, :output_history_len + i, :], all_outputs[:, :output_history_len + i, :]), dim=2)
            
            # 通过 LSTM 处理数据
            lstm_out, _ = self.lstm(combined_input)
            
            # 取 LSTM 输出的最后一个时间步
            lstm_out = lstm_out[:, -1:, :]  # 保留 batch 和特征维度，选择最后时间步的输出
            
            # 扩展并拼接 VELMA 参数
            velma_params_expanded = velma_params.unsqueeze(1).repeat(1, lstm_out.size(1), 1)
            combined_input_final = torch.cat((lstm_out, velma_params_expanded), dim=2)
            
            # 全连接层计算
            fc_out = self.fc_layers(combined_input_final.view(-1, combined_input_final.size(-1))).view(combined_input_final.size(0), 1, -1)
            
            # 生成最终输出
            current_output = self.output_layer(fc_out)  # [batch_size, 1, output_size]
            
            # 更新输出张量
            all_outputs[:, output_history_len + i:output_history_len + i + 1, :] = current_output

        # 返回最终输出
        return all_outputs


class SurrogateModel_simple_TCN(nn.Module):
    """
    使用 TCN 作为时间序列编码器的简化代理模型，并结合 VELMA 参数进行预测。
    
    参数：
    - time_series_input_size: 时间序列数据的输入特征数。
    - tcn_channels: TCN 的每层通道数列表。
    ...
    - dropout_prob: dropout 的概率。
    """
    def __init__(self, time_series_input_size=2, velma_param_size=1, 
                 tcn_channels=[64, 64, 64], kernel_size=2, dropout_prob=0.2,
                 fc_hidden_dims=[256, 128, 64], output_size=1, use_dropout=True):
        super(SurrogateModel_simple_TCN, self).__init__()
        ...
        for i in range(num_levels):
            dilation = 2 ** i
            in_channels = time_series_input_size if i == 0 else tcn_channels[i - 1]
            out_channels = tcn_channels[i]
            layers.append(nn.Conv1d(in_channels, out_channels, kernel_size=kernel_size, stride=1, 
        ...
        # TCN 输出和 VELMA 参数的组合输入大小
        self.combined_input_size = tcn_channels[-1] + velma_param_size
    ...
    def forward(self, time_series, velma_params):
        # TCN expects input in shape [batch_size, channels, seq_len]
        time_series = time_series.permute(0, 2, 1)  # 调整形状以适应 TCN 输入

        # 通过 TCN 处理时间序列
        tcn_out = self.tcn(time_series)  # 输出形状: [batch_size, tcn_channels[-1], seq_len]
        tcn_out = tcn_out.permute(0, 2, 1)  # 调整形状为 [batch_size, seq_len, tcn_channels[-1]]
        
        # 扩展并将 VELMA 参数与 TCN 输出拼接
        velma_params_expanded = velma_params.unsqueeze(1).repeat(1, tcn_out.size(1), 1)
        combined_input = torch.cat((tcn_out, velma_params_expanded), dim=2)
...
class SurrogateModel_TCN(nn.Module):
    """
    使用多变量 TCN 的代理模型，直接在模型内部实现多步预测，生成完整的时间序列输出。
    
    参数：
    - time_series_input_size: 时间序列数据的输入特征数（多变量）。
    - output_history_size: 使用的历史输出步长。
    - tcn_hidden_size: TCN 的卷积核数量。
    - velma_param_size: VELMA 参数的数量。
    - num_tcn_layers: TCN 层数。
    ...
    - dropout_prob: dropout 的概率。
    """
    def __init__(self, time_series_input_size=2, tcn_hidden_size=64, 
                 velma_param_size=1, num_tcn_layers=3, kernel_size=3, 
                 fc_hidden_dims=[128, 64], output_size=1, use_dropout=True, dropout_prob=0.2):
        super(SurrogateModel_TCN, self).__init__()
        ...
        for i in range(num_tcn_layers):
            in_channels = self.time_series_input_size if i == 0 else tcn_hidden_size
            tcn_layers.append(nn.Conv1d(in_channels=in_channels, out_channels=tcn_hidden_size, 
                                        kernel_size=kernel_size, padding=(kernel_size - 1) * dilation_size, 
                                        dilation=dilation_size))
            tcn_layers.append(nn.ReLU())
            if use_dropout:
                tcn_layers.append(nn.Dropout(dropout_prob))
            dilation_size *= 2  # 指数增长膨胀因子
        self.tcn = nn.Sequential(*tcn_layers)
        
        # 批归一化层
        self.batch_norm = nn.BatchNorm1d(tcn_hidden_size)

        # TCN 输出与 VELMA 参数的组合输入大小
        self.combined_input_size = tcn_hidden_size + velma_param_size
    ...
    def forward(self, time_series, output_history, velma_params):
        ...
        for i in range(time_series_len-output_history_len):
            # 准备当前时间步的输入
            combined_input = torch.cat((time_series[:, :output_history_len + i, :], all_outputs[:, :output_history_len + i, :]), dim=2).permute(0, 2, 1)
            
            # TCN 和批归一化
            tcn_out = self.batch_norm(self.tcn(combined_input)).permute(0, 2, 1)
            
            # 扩展并拼接 VELMA 参数
            velma_params_expanded = velma_params.unsqueeze(1).repeat(1, tcn_out.size(1), 1)
            combined_input_final = torch.cat((tcn_out, velma_params_expanded), dim=2)
            
            # 全连接层计算
            fc_out = self.fc_layers(combined_input_final.view(-1, combined_input_final.size(-1))).view(*combined_input_final.size()[:-1], -1)
            
            # 生成最终输出
            current_output = self.output_layer(fc_out)  # [batch_size, seq_len, output_size]
            
            # 更新输出张量
            all_outputs[:, output_history_len + i:output_history_len + i + 1, :] = current_output[:, -1:, :]

        # 返回最终输出
        return all_outputs


class SurrogateModel_Transformer(nn.Module):
    def __init__(self, time_series_input_size=2, transformer_d_model=64, 
                 velma_param_size=1, num_transformer_layers=3, 
                 fc_hidden_dims=[128, 64], output_size=1, use_dropout=True, dropout_prob=0.2, max_len=5000):
        super(SurrogateModel_Transformer, self).__init__()
        ...
        # 输入转换层
        self.input_fc = nn.Linear(self.time_series_input_size, transformer_d_model)

        # 创建位置编码并直接添加到输入层
        self.positional_encoding = self.create_positional_encoding(max_len, transformer_d_model)

        # Transformer
        self.transformer = nn.Transformer(d_model=transformer_d_model, nhead=4, 
                                          num_encoder_layers=num_transformer_layers,
                                          num_decoder_layers=num_transformer_layers,
                                          dim_feedforward=256, dropout=dropout_prob)

        # Transformer 输出与 VELMA 参数的组合输入大小
        self.combined_input_size = transformer_d_model + velma_param_size
    ...
    def forward(self, time_series, output_history, velma_params):
        ...
        # 将时间序列和历史输出结合并通过输入层
        combined_input = torch.cat((time_series, all_outputs), dim=2)
        combined_input = self.input_fc(combined_input)  # shape [batch_size, seq_len, d_model]

        # 添加位置编码
        combined_input += self.positional_encoding[:, :combined_input.size(1), :].to(combined_input.device)

        # 调整形状以适应 Transformer 输入
        combined_input = combined_input.permute(1, 0, 2)  # [seq_len, batch_size, d_model]

        # 通过 Transformer 编码器和解码器
        ...
        for i in range(time_series_len - output_history_len):
            ...
            transformer_step_out = transformer_output[:, output_history_len + i:output_history_len + i + 1, :]

            # 扩展并拼接 VELMA 参数
            velma_params_expanded = velma_params.unsqueeze(1).repeat(1, transformer_step_out.size(1), 1)
            combined_input_final = torch.cat((transformer_step_out, velma_params_expanded), dim=2)
...
class SurrogateModel_TCN_Transformer(nn.Module):
    def __init__(self, time_series_input_size=2, tcn_hidden_size=64, velma_param_size=1,
                 num_tcn_layers=3, kernel_size=3, fc_hidden_dims=[128, 64], 
                 output_size=1, num_transformer_layers=2, use_dropout=True, dropout_prob=0.2):
        super(SurrogateModel_TCN_Transformer, self).__init__()
        ...
        for i in range(num_tcn_layers):
            ...
            tcn_layers.append(nn.Conv1d(in_channels=in_channels, out_channels=tcn_hidden_size, 
                                        kernel_size=kernel_size, padding=padding, dilation=dilation_size))
            tcn_layers.append(nn.ReLU())
            if use_dropout:
                tcn_layers.append(nn.Dropout(dropout_prob))
            dilation_size *= 2
        ...
        # 用于调整 output_history 的特征维度，使其匹配 tcn_hidden_size
        self.history_fc = nn.Linear(output_size, tcn_hidden_size)

        # Transformer 解码器
        self.transformer_decoder_layer = nn.TransformerDecoderLayer(d_model=tcn_hidden_size, nhead=4, 
                                                                    dim_feedforward=256, dropout=dropout_prob)
    ...
    def add_positional_encoding(self, x):
        ...
        position = torch.arange(0, seq_len, dtype=torch.float32, device=x.device).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, d_model, 2, device=x.device) * (-math.log(10000.0) / d_model))
        pe = torch.zeros(seq_len, d_model, device=x.device)
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)
        return x + pe.unsqueeze(1)  # [seq_len, batch_size, d_model]

    def forward(self, time_series, output_history, velma_params):
        # 通过 TCN 编码器处理时间序列
        time_series = time_series.permute(0, 2, 1)  # 调整形状以适应 TCN 输入
        tcn_out = self.batch_norm(self.tcn(time_series))  # [batch_size, tcn_hidden_size, seq_len]
        tcn_out = tcn_out.permute(2, 0, 1)  # 调整为 [seq_len, batch_size, tcn_hidden_size]
        
        # 将 output_history 的特征维度映射到 tcn_hidden_size，并扩展到 time_series 的长度
        output_history = self.history_fc(output_history)  # [batch_size, history_len, tcn_hidden_size]
        output_history = output_history.permute(1, 0, 2)  # [history_len, batch_size, tcn_hidden_size]
        
        # 使用零填充扩展 output_history 到 time_series 长度
        seq_len = tcn_out.size(0)
        if output_history.size(0) < seq_len:
            padding = torch.zeros(seq_len - output_history.size(0), *output_history.shape[1:], device=output_history.device)
            full_output = torch.cat((output_history, padding), dim=0)  # [seq_len, batch_size, tcn_hidden_size]
        else:
            full_output = output_history

        # 添加位置编码
        full_output = self.add_positional_encoding(full_output)
        tcn_out = self.add_positional_encoding(tcn_out)
        
        # 生成掩码以阻止 Transformer 看到未来时间步
        tgt_mask = nn.Transformer.generate_square_subsequent_mask(full_output.size(0)).to(full_output.device)
        
        # 使用 Transformer 解码器生成完整的多步输出序列
        transformer_out = self.transformer_decoder(full_output, tcn_out, tgt_mask)  # [seq_len, batch_size, tcn_hidden_size]
        transformer_out = transformer_out.permute(1, 0, 2)  # [batch_size, seq_len, tcn_hidden_size]

        # 扩展并拼接 VELMA 参数
        velma_params_expanded = velma_params.unsqueeze(1).repeat(1, transformer_out.size(1), 1)
        combined_input_final = torch.cat((transformer_out, velma_params_expanded), dim=2)

        # 全连接层计算，逐时间步映射到 output_size
        fc_out = self.fc_layers(combined_input_final.view(-1, combined_input_final.size(-1))).view(
            *combined_input_final.size()[:-1], -1)  # [batch_size, seq_len, last_fc_dim]

        # 生成完整序列的最终输出
        output = self.output_layer(fc_out)  # [batch_size, seq_len, output_size]
...
def train_model(main_model, surrogate_model, optimizer_main, optimizer_surrogate, dataloader, 
                criterion=nn.MSELoss(), train_main=True, surrogate_epochs=None, main_epochs=None, epochs=10,
                save_path=None, grad_clip_value=None, device='cuda', plot_loss=True,surrogate_model_type='tcn'):
    """
    ...
    for epoch in range(surrogate_epochs):
        ...
        for images, time_series, xml_params, real_outputs, velma_params, _ in dataloader:
            ...
            if surrogate_model_type in ['tcn', 'lstm','transformer','tcn_transformer']:
                predicted_outputs = surrogate_model(time_series, real_outputs[:,0:1,:],velma_params)
            elif surrogate_model_type in ['simple_lstm', 'simple_tcn']:
...
def optimize_and_visualize(main_model, surrogate_model, dataloader, observed_data, scalers, params_range, criterion=nn.MSELoss(), lr=0.01, epochs=10, use_main_model=False, device='cuda', plot_prediction=True, surrogate_model_type = 'tcn'):
    """
    ...
    images, time_series, xml_params, outputs, velma_params, observed_time_series = (
    item[0:1].to(device) for item in next(iter(dataloader))
)
    ...
    if use_main_model and main_model is not None:
        ...
        initial_params = torch.tensor(np.array(
            [np.random.uniform(low, high,1) for low, high in params_range.values()]),
    ...
    for epoch in range(epochs):
        optimizer.zero_grad()
        if surrogate_model_type in ['tcn', 'lstm','transformer','tcn_transformer']:
            predicted_outputs = surrogate_model(observed_time_series, outputs[:,0:1,:],optimal_params)
        elif surrogate_model_type in ['simple_lstm', 'simple_tcn']:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', type=int, default=30, help='Total number of training epochs for the model')
    parser.add_argument('--batch_size', type=int, default=32, help='Size of each batch for training and data loading')
    ...
    # Dataset parameters
    surrogate_model_type = 'simple_lstm'
    ...
    if surrogate_model_type == 'tcn':
        surrogate_model = SurrogateModel_TCN(velma_param_size=len(params_range), output_size=len(required_columns)).to(device)
    elif surrogate_model_type == 'lstm':
        surrogate_model = SurrogateModel_LSTM(velma_param_size=len(params_range), output_size=len(required_columns)).to(device)
    elif surrogate_model_type == 'transformer':
        surrogate_model = SurrogateModel_Transformer(velma_param_size=len(params_range), output_size=len(required_columns)).to(device)
    elif surrogate_model_type == 'tcn_transformer':
        surrogate_model = SurrogateModel_TCN_Transformer(velma_param_size=len(params_range), output_size=len(required_columns)).to(device)
    elif surrogate_model_type == 'simple_lstm':
        surrogate_model = SurrogateModel(velma_param_size=len(params_range), output_size=len(required_columns)).to(device)
    elif surrogate_model_type == 'simple_tcn':
        surrogate_model = SurrogateModel_simple_TCN(velma_param_size=len(params_range), output_size=len(required_columns)).to(device)
    ...
    train_model(
        ...
        main_epochs=10, save_path=args.save_path, plot_loss=False,surrogate_model_type=surrogate_model_type
    )
    ...
    predicted_outputs, best_params, indices = optimize_and_visualize(
        ...
        use_main_model=False, device=device, plot_prediction=True,surrogate_model_type = surrogate_model_type
    )
